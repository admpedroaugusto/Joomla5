...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" exclude-result-prefixes="php">
	...
	<xsl:template name="font">
		...
		<xsl:if test="//config/basefont/@value != 'none' and //config/basefont/@value != ''">
			<xsl:value-of select="php:function( 'tplDefault8::LoadFont', $basefont)"/>
			<xsl:value-of select="php:function( 'tplDefault8::ApplyBaseFont', $basefont)"/>
		</xsl:if>
		...
		<xsl:if test="$basefont != $specialfont and //config/specialfont/@value != '' and //config/specialfont/@value != 'none'">
			<xsl:value-of select="php:function( 'tplDefault8::LoadFont', $specialfont)"/>
			<xsl:value-of select="php:function( 'tplDefault8::ApplyFont', $specialfont)"/>
		</xsl:if>
