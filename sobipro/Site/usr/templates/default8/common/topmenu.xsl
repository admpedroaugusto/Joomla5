...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" exclude-result-prefixes="php">
	...
	<xsl:template name="topMenu">
		...
		<xsl:if test="count(//menu/*) > 0">
			<xsl:choose>
				<xsl:when test="//config/topmenu/@value = 'standard' or //config/topmenu/@value = 'standard-only' or //config/topmenu/@value = 'standard-without'">
					...
					<nav class="navbar navbar-expand-md navbar-dark sp-topmenu standard" role="navigation">
						<div class="container-fluid">
							<button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#topmenu" aria-controls="topmenu"
							        aria-expanded="false" aria-label="{php:function( 'SobiPro::TemplateTxt', 'ACCESSIBILITY.FE.TOGGLE_NAVIGATION' )}">
							...
							<div id="topmenu" class="collapse navbar-collapse">
								...
								<xsl:if test="//menu/search and $searchbox = 'true' and (//config/topmenu/@value = 'standard' or //config/topmenu/@value = 'standard-only')">
									<form class="sp-quicksearch-form d-flex my-2 my-lg-0 pl-2 pr-2">
										<input type="search" id="sp-quicksearch" name="sp_search_for" autocomplete="off"
										       ...
										       placeholder="{php:function( 'SobiPro::TemplateTxt', 'SH.SEARCH_FOR' )}"
										       aria-label="{php:function( 'SobiPro::TemplateTxt', 'ACCESSIBILITY.FE.SEARCH_FOR' )}"/>
										<xsl:if test="//config/topmenu/@value = 'standard'">
	...
	<xsl:template name="navigationbar">
		...
		<ul class="{$nav}" role="menubar" aria-label="{php:function( 'SobiPro::TemplateTxt', 'ACCESSIBILITY.FE.SECTION_NAVIGATION', string(section) )}/>">
					...
					<xsl:if test="string-length(//config/redirectlogin/@value) > 0">
						<li>
							<a href="{//config/redirectlogin/@value}" role="menuitem" class="nav-link">
								<xsl:value-of select="php:function( 'SobiPro::Icon', 'plus-circle', $font )" disable-output-escaping="yes"/>
								<xsl:text> </xsl:text>
								<xsl:value-of select="php:function( 'SobiPro::TemplateTxt', 'MN.ADD_ENTRY' )"/>
							</a>
						</li>
					</xsl:if>
				</xsl:otherwise>
			...
			<xsl:if test="//menu/search">
				<li class="nav-item">
					<a href="{//menu/search/@url}/?sparam=show" tabindex="0" role="menuitem" class="nav-link">
	...
	<xsl:template name="linkbar">
		<div class="sp-topmenu linkbar" role="menubar">
			...
			<ul>
				...
				<xsl:if test="//menu/search">
					<li>
						<a class="sp-topmenu-link" href="{//menu/search/@url}/?sparam=show" tabindex="0" role="menuitem">
	...
	<xsl:template name="buttonbar">
		<div class="sp-topmenu buttonbar" role="menubar">
			...
			<div class="search">
				<xsl:if test="//menu/search">
					<a href="{//menu/search/@url}/?sparam=show" class="btn btn-delta" tabindex="0" role="menuitem" data-role="button">
						<xsl:value-of select="//menu/search"/>
					</a>
				</xsl:if>
			</div>
